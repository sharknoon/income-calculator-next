...
export function CalculationEditor({ component }: CalculationEditorProps) {
  ...
  const [calculationCode, setCalculationCode] = useState(period?.calculate || "new BigNumber(0)")
  ...
  const handleRemoveDependency = (dependencyId: string) => {
    ...
    const newPeriod = { ...period }
    newPeriod.dependencies = newPeriod.dependencies.filter((id) => id !== dependencyId)
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Use JavaScript with BigNumber.js to create your calculation formula. Access input values using the input ID,
          e.g., <code>inputs.hourlyRate * inputs.hoursWorked</code>
        </p>
      ...
      <div className="space-y-2">
        ...
        <div className="border rounded-md p-4">
          <p className="text-sm mb-2">Select other components that this calculation depends on:</p>

          {getAvailableDependencies().length === 0 ? (
            <p className="text-sm text-muted-foreground">No other components available</p>
          ) : (
            <div className="space-y-2">
          ...
          {period?.dependencies.length > 0 && (
            <div className="mt-4">
              <p className="text-sm mb-2">Access dependency values in your calculation using:</p>
